-- place_order: carry bundle grouping + gift lines (collection-requirement.md §5.2).
--
-- Extends the 0025 version with three additions, fully backward-compatible:
--   • an optional per-item `unit_price` override — bundle member lines arrive
--     already discounted, and the free gift arrives at 0₮; ordinary items send
--     none, so the variant's list price is still used.
--   • `is_gift` and `collection_id` / `collection_name` persisted on order_items
--     so a placed order can be grouped and the gift shown as free.
-- Coupon + loyalty logic is unchanged: the coupon still applies on the bundle-
-- discounted subtotal (davhar, §6.3), and the gift (0₮) doesn't move it.
