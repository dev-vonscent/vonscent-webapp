-- Захиалга дээр «үндсэн үнэ» ба «багцын хямдрал»-ыг сэргээн харуулах.
--
-- Асуудал: багцын гишүүн мөр нь `order_items.unit_price`-д АЛЬ ХЭДИЙН
-- хямдарсан үнээрээ бичигддэг (0028a), `orders.subtotal` нь тэдгээрийн
-- нийлбэр, харин `orders.discount` нь ЗӨВХӨН купон (place_order). Тиймээс
-- захиалга үүсмэгц багцаас хэдэн төгрөг хэмнэснийг хаанаас ч тооцох
-- боломжгүй болдог: checkout дээр «Багцын хямдрал» гэж харсан мөр нь
-- pay / order / account / admin / и-мэйл дээр алга болж, мөрүүдийн дүн нь
-- «яагаад ийм тоо гарав» гэдгийг тайлбарлахаа больдог.
--
-- Засвар: хямдраагүй үнийг мөр бүр дээр (`order_items.list_price`) ба
-- захиалгын хэмжээнд (`orders.gross_subtotal`) хамт хадгална. Хоёулаа
-- nullable — хуучин захиалгууд утгагүй үлдэх бөгөөд уншигч тал тэр үед
-- өмнөх шигээ ганц «Барааны дүн» мөр харуулна.
--
-- Тоон утга өөрчлөгдөхгүй: `subtotal`, `discount`, `total` бүгд хэвээр.
-- Зөвхөн ТАЙЛБАРЛАХ өгөгдөл нэмэгдэж байна.

alter table order_items
  add column if not exists list_price int check (list_price >= 0);
comment on column order_items.list_price is
  'Хямдраагүй нэгж үнэ (variant_price). Багцын гишүүнд unit_price-аас их, '
  'бусад мөрөнд түүнтэй тэнцүү. Хуучин захиалгад null.';

alter table orders
  add column if not exists gross_subtotal int check (gross_subtotal >= 0);
comment on column orders.gross_subtotal is
  'sum(list_price * qty) — багцын хямдралын ӨМНӨХ барааны дүн. '
  'gross_subtotal − subtotal = багцын хямдрал. Хуучин захиалгад null.';

-- place_order: бие нь 0096-ынхтай адил, зөвхөн `v_list` тооцоо ба хоёр шинэ
-- баганын бичилт нэмэгдэв.
...
comment on function place_order(jsonb, jsonb) is
  'Захиалга үүсгэх: савны түгжээ (0095) → нөөц түгжих → мөр бичих (үндсэн ба '
  'хямдарсан үнэ хоёуланг, 0097) → купон / V point.';
